...
def bin_str_to_bytearray(binary_str: str) -> []:
    ...
    for byte_index in range(0, len(binary_str), 8):
        start = byte_index
        byte_array.append(int(binary_str[start: start + 8], 2))  # pad to full byte
    return byte_array


def binary_string_to_morse(binary_str: str) -> str:
    """Changes binary in a string to a morse code string.
        :parameter binary_str
            string representation of binary. 111010
        :returns str
            string representation of morse code -.
    """
    ...
    return morse_str.rstrip("0/")  # there may be artifacts from padding to full bytes
